...
- macOS 可开启 Agent 模式，隐藏 Dock、应用切换器以及可选的菜单栏图标，同时保持后台注入
...
- **macOS 后台运行**：在概览页开启 Agent 模式；隐藏全部入口后可从“应用程序”重新打开 Codex Tweaks 以恢复窗口，启动顺序与限制参见 [macOS Agent 模式](docs/macos-agent-mode.md)
