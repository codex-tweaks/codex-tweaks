...
<p align="center">
  在一个桌面应用中安装、启停和更新 Codex 的本地界面增强。
</p>
...
- 自动连接 Codex，并集中显示连接和运行状态
- 从本地目录、ZIP 或 Git 仓库安装功能包
- 单独启用、停用和更新每个功能包
- 自动处理包依赖和加载顺序，并在异常时指出具体原因
- 仅在新版本编译成功后切换，失败时继续保留上一个可用版本
- 查看运行日志，并在应用内检查和安装 Codex Tweaks 更新
