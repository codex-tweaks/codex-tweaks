...
function Invoke-BackendSmoke([string]$Backend, [string]$Publish, [string]$ExpectedVersion) {
    ...
    try {
        ...
        if ($null -eq $ping -or $ping.result.backend -ne 'go' -or $ping.result.protocolVersion -ne 3) {
            throw 'Backend ping response did not satisfy protocol v3.'
        }
        if ($null -eq $initialize -or $initialize.result.protocolVersion -ne 3) {
            throw 'Backend initialize response did not satisfy protocol v3.'
        }
